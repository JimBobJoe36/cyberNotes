...
def textToBin(message):
    ...
    print(f"Your encoded string is:\n {binaryString}\n File created in this folder to share.")

    # Write the message to Desktop
    iteration = 1
    # Tries to make a file with the name "binaryMessage[Number].txt", and keeps trying to make a different one.
    while True:
        try:
            theFile = open(f"C:/Users/{user}/Documents/School/Cyber201/Notes/Packet Tracer/binaryMessage{iteration}.txt", 'x')
